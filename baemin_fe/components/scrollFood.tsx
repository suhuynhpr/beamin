...
export default function ScrollBar({ items }: { items: any }) {
  ...
  return (
    <>
      ...
      >
        <div className="w-full h-[300px] flex flex-col px-4 pt-4 pb-2">
          <div className="w-full flex flex-row justify-between h-14">
            <div className="relative ml-3 text-xl font-bold mb-2">
              {items.title}
            </div>
            <Link href={""} className="relative ml-3 text-xl font-bold mb-2">
              <Button type="link">Xem thêm</Button>
            </Link>
          </div>

          <div className="w-full relative h-full flex-1">
            {currentIndex > 0 && (
              <button
                onClick={handlePrev}
